...
public class ButtonGridSection<T extends GirlSceneEntity, V> extends CustomizeSection<T> {
    ...
    // Overload for original Fabric signature that used Text instead of Component
    public ButtonGridSection(T entity, T previewEntity, String title, String groupId, V[] options, int columns,
                             java.util.function.Function<V, net.minecraft.network.chat.Component> labelProviderLegacy,
                             java.util.function.Consumer<V> onSelect, java.util.function.Supplier<V> currentValue,
                             boolean dummy) {
        this(entity, previewEntity, title, groupId, options, columns, labelProviderLegacy::apply, onSelect, currentValue);
    }
    ...
    public int render(CustomizeScreen<T> screen, LayoutConfig layout, int currentY) {
        StringWidget titleWidget = new StringWidget(layout.centerX, currentY, layout.contentWidth, 20,
                Component.literal(title), net.minecraft.client.Minecraft.getInstance().font);
