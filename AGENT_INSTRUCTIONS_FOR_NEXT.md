...
> Всё ниже собрано и проверено CI (последний зелёный прогон — `31618275392`).
...
### 5.4. Переноска: посадка на плечо и фикс вращения

Первая версия переноски ставила девушку **перед** игроком (а в первом лице — сбоку в half
scale), из-за чего она выглядела висящей в паре блоков. Плюс она крутилась на месте.

- **Вращение.** `LookAtPlayerGoal` и `RandomLookAroundGoal` — ванильные цели, они ничего
  не знают о том, что сущность стала пассажиром, и продолжали выбирать цель взгляда и
  вертеть её на плече. Обе обёрнуты подклассом с проверкой `isCarried()`, а в `tick()`
  поворот жёстко приравнивается к повороту носильщика. Писать нужно **все шесть** полей,
  включая `*O` (прошлый тик), иначе рендерер интерполирует между старым и новым yaw и
  девушка дрожит.
- **Посадка.** Теперь сидит на правом плече вплотную, масштаб `0.62`. Колени поджаты
  вручную через кости `legL`/`legR`/`shinL`/`shinR` — готовой анимации переноски нет ни у
  одного рига, кроме Мики (проверено по всем 14 geo-файлам, что эти кости везде есть).
  У Мики своя `carry_slow1`, поэтому для неё ручная поза **не** применяется, иначе они
  спорили бы за одни и те же кости. Остальные риги в переноске играют `idle`, а не `sit`,
  по той же причине.
- Кости в GeckoLib общие между всеми девушками на одном риге, поэтому поза сбрасывается
  для каждой непереносимой девушки (`clearCarryPose`) — иначе одна на руках усадила бы всех.

**Правка после теста в игре.** Первый вариант посадки давал два бага:

- *Летала над игроком.* Высота применялась **дважды**: точка крепления поднимала её
  (`playerY + 1.8 − 0.7 = 1.1`), и рендерер добавлял сверху ещё `+0.62` → корень модели
  на 1.72 блока. Теперь высоту задаёт **только** `getVehicleAttachmentPoint`
  (`1.62 − 0.42 = 1.2`, уровень плеча), а `SHOULDER_UP = 0`. Знак `SHOULDER_FORWARD` тоже
  был обратным для этого рига и выталкивал её за спину.
- *Поворачивалась всем телом за взглядом.* Поворот был привязан к `player.getYRot()` — у
  игрока это yaw **головы/взгляда**, он меняется от каждого движения мышью. Сейчас
  используется `player.yBodyRot` — поворот **корпуса**; ваниль подтягивает его к взгляду
  по 0.3 за тик с ограничением (`LivingEntity` ~строка 2566), поэтому девушка едет на
  корпусе, а не дёргается за мышью. Голова при этом остаётся свободной — ей занимается
  head tracking в рендерере.

**Вторая правка после теста — главная причина.** Девушка «левитировала на месте»: игрок
шёл и поворачивался, а она оставалась висеть в мире (её было видно сзади). Это **не**
подбор констант, а механика тика:

- позиционирует пассажира `Entity#rideTick` → `vehicle.positionRider(this)`;
- но `ClientLevel#tickEntities` **пропускает** всех, у кого `isPassenger()` (строка 259),
  и доходит до пассажиров только через `tickPassenger`, который требует наличия сущности
  в клиентском `tickingEntities`;
- девушка, загруженная в мир **до** того, как её взяли на руки, туда не попадает →
  `positionRider` на клиенте не вызывается вообще → она стоит там, где её last-known
  позицию оставил сервер.

Поэтому позиция и поворот теперь пересчитываются **в рендерере** (`snapToCarrier`),
той же формулой, что и `positionRider`. Рендер идёт каждый кадр независимо от тика, так
что пропустить его нельзя. Обязательно пишутся и «предыдущие» координаты (`xo/yo/zo`,
`xOld/yOld/zOld`) — иначе рендерер интерполирует от старой позиции и она «размазывается».

Логика в `TameableGirlEntity#tick` оставлена как есть — она работает на сервере
(там пассажиры тикаются) и служит источником истины.

**Высота (исправленная арифметика).** `Player` не объявляет attachment `PASSENGER`,
поэтому берётся фолбэк `AT_HEIGHT` = полная высота **1.8** (а не 1.62 eye height, как
я ошибочно посчитал в прошлый раз). `1.8 − 0.6 = 1.2` — уровень плеча.

> Если понадобится ещё подвинуть — все числа константами наверху `GirlRenderer`
> (`SHOULDER_RIGHT`, `SHOULDER_UP`, `SHOULDER_FORWARD`, `CARRY_SCALE`, углы `CARRY_*`),
> высота — в `getVehicleAttachmentPoint` в `TameableGirlEntity`. Важно: высоту менять
> в **одном** месте, иначе снова сложится вдвое.
...
Важно: броня лежит в собственном контейнере девушки (`GirlInventory`, слоты 1–4), а **не**
в ванильных слотах экипировки — `getItemBySlot` для этих сущностей всегда пустой.
Для этого есть `getArmorStack(EquipmentSlot)`.
